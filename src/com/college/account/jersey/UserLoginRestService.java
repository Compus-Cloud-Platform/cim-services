...
public class UserLoginRestService {
    ...
    @Path("/delete")
    @Consumes({MediaType.APPLICATION_JSON})
	public String delUserGet(@QueryParam("id") String id){
		...
		usersSearch = ServiceFactoryBean.getUserService().searchByid(idtemp);
		if(null == usersSearch){
			resultMap.put("ack", "failure");
			dataMap.put("code", 1005);
			dataMap.put("field", "id");
			dataMap.put("description", "id not find");
			resultMap.put("errors", dataMap);;
		}else{
			
			usersextSearch = ServiceFactoryBean.getUserServiceExt().search(usersSearch.getLoginId());
			
			/* delete */
			ServiceFactoryBean.getUserService().deleteUsers(usersSearch.getId());
			ServiceFactoryBean.getUserServiceExt().delete(idtemp);
    ...
    @Path("/delete")
    @Consumes({MediaType.APPLICATION_JSON})
	public String delUserPost(@QueryParam("id") String id){
		return delUserGet(id);
	}
	
	@GET
    @Path("/search")
    @Consumes({MediaType.APPLICATION_JSON})
	public String getUserGet(@QueryParam("id") String id){
		Integer idtemp = Integer.valueOf(id);
		Users usersSearch = null;
		UsersExt usersextSearch = null;
		Map resultMap = new HashMap();
		...
		if(null == usersSearch){
			...
			Map userMap = Obj2Map.toMap(usersSearch, Users.class);
			Map userextMap = Obj2Map.toMap(usersextSearch, UsersExt.class);
			
			userextMap.remove("id");
			userMap.putAll(userextMap);
